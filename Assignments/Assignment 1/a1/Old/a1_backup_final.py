...
def check_win(puzzle: str, solution: str) -> bool:
    ...
    solution_list = convert_str_to_list(solution)
    solution_list[-1] = EMPTY               # Makes last character a blank
    solution = convert_list_to_str(solution_list)
...
def swap_position(puzzle: str, from_index: int, to_index: int) -> str:
    ...
    puzzle_list = convert_str_to_list(puzzle)
    ...
    puzzle = convert_list_to_str(puzzle_list)
...
def move(puzzle: str, direction: str) -> Optional[str]:
    ...
    if direction == UP:
        ...
        if white_index % word_len != 0 and (white_index - 1) >= 0:
            move_piece = white_index - 1
            valid_move = True
    elif direction == RIGHT:
        if (white_index + 1) % word_len != 0 \
                and (white_index + 1) <= len(puzzle):
            move_piece = white_index + 1
            valid_move = True
...
def convert_str_to_list(input_string: str) -> list:
    """
    Splits a string into a list

    Parameters:
        input_string (str): an input string which is then converted into a
                            list

    Returns:
        (list): a returned list matching input_string with 1 character per
                position
    """
    
    returned_list = []
    for i in range(len(input_string)):
        returned_list.append(input_string[i])
...
def convert_list_to_str(input_list: list) -> str:
    """
    Converts a list into a single string using join function

    Parameters:
        input_list (list): an input list which is then converted into a
                           string

    Returns:
        (str): a returned string matching input_list with no joining
               character
    """
...
def main():
    ...
    """Main execution of code for the program"""
    ...
    while playing:
        ...
        # Main Game Loop
        while not win:                  # While user hasn't solve puzzle
            # Print out grids
            print("Solution:")
            ...
            if user_input == HELP:          # User requests controls
                ...
                if play_again == "Y" or play_again == "":
                    ...
                    print(BYE)
                    return
            elif user_input == "CHAD":      # Cheat code
                puzzle = solution[:-1] + EMPTY
                print("You have entered the 'CHAD' cheat code!\n")
